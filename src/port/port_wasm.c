...
int getTimeInIso8601(char *pBuf, size_t uBufSize)
{
    ...
    else
    {
        ...
        printf("%s(): xTimeUtcNow = %llu\n", __FUNCTION__, xTimeUtcNow);
        
        char* ptr = (char*)&xTimeUtcNow;
...
uint64_t getEpochTimestampInMs(void)
{
    ...
    printf("%s(): tv_sec = 0x%llx, tv_usec = 0x%llx, timestamp = 0x%llx\n", __FUNCTION__, tv.tv_sec, tv.tv_usec, timestamp);
