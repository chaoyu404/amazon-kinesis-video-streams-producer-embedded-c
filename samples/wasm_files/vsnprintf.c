...
char strerror_value[32] = {'\0'};
char *strerror(int errnum)
{
    snprintf(strerror_value, 32, "%d", errno);
...
{
    int ret = 0;
    char buffer[VPRINTF_BUFFER_SIZE] = {'\0'};

    //printf("%s() is called: format = %s\n", __FUNCTION__, format);

    ret = vsnprintf(buffer, VPRINTF_BUFFER_SIZE, format, ap);
